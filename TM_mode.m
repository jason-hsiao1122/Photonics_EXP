...
% =========================
data = readmatrix(filename, 'Sheet', sheetName, 'Range', 'G19:K29');
...
saveas(gcf, 'reflection_law_TM.png');
...
saveas(gcf, 'snell_law_TM.png');

%% =========================
%  Figure 3: TM mode Fresnel 反射率與穿透率
% =========================
% R = I_r / (I_r + I_t)
% T = I_t / (I_r + I_t)
%
...
theta_t_plot = asind((n1/n2) .* sind(theta_plot));
...
plot(theta_plot, R_TM, 'LineWidth', 2);
plot(theta_plot, T_TM, 'LineWidth', 2);

grid on;
xlabel('\theta_i (degree)');
ylabel('Reflectance / Transmittance');
title('TM Mode Fresnel Reflectance and Transmittance');
...
saveas(gcf, 'raw_intensity_points_TM.png');
